...
if __name__=='__main__':
    ...
    exportpatientframe.to_excel("ComedicationPID.xlsx", sheet_name="PatientID", index=False);
    '''
    ...
    '''
    #input("Please place the master dataset (name=master1) in the program's folder and press ENTER to continue.");
    ...
    patientsarray=exportpatientframe['P_ID'].values;
    patientslist=list(patientsarray);
    patientslist=list(map(int, patientslist));
    usewindowscred=True;
    if usewindowscred:
        ...
        cnxn=pyodbc.connect('DRIVER={SQL Server};'
                            'SERVER=BRUSSQL05;'
                            'DATABASE=LRX_PANEL;'
                            'UID=vijay.viswanathan;'
                            'PWD=Winter13;');
    crsr=cnxn.cursor();
    placeholder='?';
    placeholders=','.join(placeholder*len(patientslist));
    sql='select a.*,b.combomol,c.prod,c.pack,d.psize \
    from lrx_panel.dbo.tbl_data a \
    left join cpi.dbo.vw_combomol b on \
    a.fcc = b.fcc \
    left join cpi.dbo.allinfo c on \
    a.fcc = c.fcc \
    left join CPI.DBO.PPACK d on \
    a.fcc = d.fcc \
    where P_id in (%s)'%placeholders;
    penultimateframe=psql.read_sql(sql, cnxn, params=tuple(patientslist));
    cnxn.close();
    penultimateframe.to_excel("master.xlsx", sheet_name='Comedication', index=False);
    ...
    patproframe.to_excel("PatProPID.xlsx", sheet_name='PatientID', index=False);
    patientsarray2=patproframe['P_ID'].values;
    patientslist2=list(patientsarray2);
    patientslist2=list(map(int, patientslist2));
    usewindowscred=True;
    if usewindowscred:
    ...
    crsr2=cnxn2.cursor();
    placeholder='?';
    placeholders=','.join(placeholder*len(patientslist2));
    sql2='select * \
    from LRx_Panel.dbo.Tbl_PatientProfil \
    where P_id in (%s)'%placeholders;
    patprofilframe=psql.read_sql(sql2, cnxn2, params=tuple(patientslist2));
    cnxn2.close();
    patprofilframe.to_excel("patprofil.xlsx", sheet_name='PatProfil', index=False);
    ...
    summaryframe=pandas.DataFrame(columns=['P_ID', 'ANTI-TNF', 'PACKS_1ST_MONTH', \
                                           ...
                                           'PACKS_12_MONTHS', 'TOTAL_PACKS', 'TOTAL_MG_CONSUMED', 'TOTAL TREATMENT IN STD PACKS(40MGx2)', \
                                           '1ST_TRANS_DATE_ON_HUMIRA', \
                                           'LAST_TRANS_DATE_ON_HUMIRA', \
                                           'PATPROFIL', \
                                           'SINGLE_PURCHASE', 'SHARE OF PACKS PER WEEK', 'SHARE OF STD PACKS PER WEEK', 'PROPOSED_FLAG']);
    ...
    for patientframe in hsplist:
        ...
        for patient_id, patient_stats in pfgroup:
            ...
            for jbuydate in tempframe['TransactionDate']:
                ...
                j+=1;
            # TOTAL_MG_CONSUMPTION & TOTAL TREATMENT IN STD PACKS(40MGx2)
            total_mg_consumed=0;
            for index, rx in tempframe.iterrows():
    ...
    finalsummaryframe=pandas.DataFrame(columns=['P_ID', 'ANTI-TNF', 'PACKS_1ST_MONTH', \
                                                ...
                                                'SINGLE_PURCHASE', 'SHARE OF PACKS PER WEEK', 'SHARE OF STD PACKS PER WEEK', 'PROPOSED_FLAG', 'UNITS_BASED_FLAG']);
    ...
    for index, rx in finalsummaryframe.iterrows():
        ...
        if rx['ANTIBIOTIC'] == 'True':
            if int(rx['SHARE OF STD PACKS PER WEEK']) >= 45:
                rx['FINAL_FLAG']='HS';
            elif 40 <= int(rx['SHARE OF STD PACKS PER WEEK']) < 45:
    ...
    finalsummaryframe2[['P_ID', 'PACKS_1ST_MONTH', 'OBSERVATION_PERIOD', 'PANEL_PERIOD', \
                        ...
                        'TOTAL TREATMENT IN STD PACKS(40MGx2)', '1ST_TRANS_DATE_ON_HUMIRA', \
                        ...
                        =finalsummaryframe2[['P_ID', 'PACKS_1ST_MONTH', 'OBSERVATION_PERIOD', 'PANEL_PERIOD', \
                                             ...
                                             'TOTAL TREATMENT IN STD PACKS(40MGx2)', '1ST_TRANS_DATE_ON_HUMIRA', \
                                             'LAST_TRANS_DATE_ON_HUMIRA', 'PATPROFIL', 'SHARE OF PACKS PER WEEK', \
                                             'SHARE OF STD PACKS PER WEEK']].apply(pandas.to_numeric, errors='coerce');
